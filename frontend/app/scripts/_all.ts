/// <reference path="../../typings/tsd.d.ts" />

/// -----------Common module-----------
/// <reference path="../app.ts" />
/// <reference path="common/customMarker.ts" />
/// <reference path="common/directives/languageDropdown.directive.ts" />
/// <reference path="common/directives/currencyDropdown.directive.ts" />
/// <reference path="common/directives/timeline.directive.ts" />
/// <reference path="common/directives/timelineSegment.directive.ts" />
/// <reference path="common/directives/dropzone.directive.ts" />
/// <reference path="common/directives/mapMarker.directive.ts" />
/// <reference path="common/directives/waitingAnimation.directive.ts" />
/// <reference path="common/factories/spinnerInterceptor.factory.ts" />
/// <reference path="common/filters/currencyFormatter.filter.ts" />
/// <reference path="common/filters/capitalize.filter.ts" />
/// <reference path="common/filters/dateFormatter.filter.ts" />
/// <reference path="common/filters/durationFormatter.filter.ts" />
/// <reference path="common/filters/priceFormatter.filter.ts" />
/// <reference path="common/filters/titleExtractor.filter.ts" />
/// <reference path="common/filters/vehicleTypeToAction.filter.ts" />
/// <reference path="common/filters/vehicleTypeToIcon.filter.ts" />
/// <reference path="common/services/googleMap.service.ts" />
/// <reference path="common/services/language.service.ts" />
/// <reference path="common/services/browser.service.ts" />
/// <reference path="common/services/currency.service.ts" />
/// <reference path="common/services/inactivityDetector.service.ts" />
/// <reference path="common/services/compatibilityChecker.service.ts" />
/// <reference path="common/_common.ts" />

/// -----------Result Module -----------
/// <reference path="result/result.controller.ts" />
/// <reference path="result/result.route.ts" />
/// <reference path="result/tripApi.service.ts" />
/// <reference path="result/tripCache.service.ts" />
/// <reference path="result/itineraryHelper.service.ts" />
/// <reference path="result/tripContainer.directive.ts" />
/// <reference path="result/resultMap.directive.ts" />
/// <reference path="result/_result.ts" />

/// -----------Search Module ------------
/// <reference path="search/search.controller.ts" />
/// <reference path="search/search.route.ts" />
/// <reference path="search/suggestionAdapter.factory.ts" />
/// <reference path="search/popupTimePicker.directive.ts" />
/// <reference path="search/searchMap.directive.ts" />
/// <reference path="search/_search.ts" />

/// -----------Booking Module ------------
/// <reference path="print/print.controller.ts" />"
/// <reference path="print/print.route.ts" />
/// <reference path="print/printApi.service.ts" />
/// <reference path="print/staticMap.directive.ts" />
